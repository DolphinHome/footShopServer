...
class Format extends Controller
{
    ...
    private $variable = [
        ...
        'row_list' => [],       // 表格数据列表
        'pages' => '',       // 分页数据
        'bottom_button_select' => [],       // 底部选择操作
        'top_statistics' => [],       // 头部统计数据
    ...
    public function setSearch($fields = [], $placeholder = '', $url = '', $search_button = null)
    {
        if (!empty($fields)) {
            ...
            $this->variable['search_button'] = $search_button !== null ? $search_button : '搜索';
        }
        return $this;
    }
     ...
     * @return $this
     */
    public function addSearchColumn($name = '', $title = '', $type = '', $default = '', $param = '', $class = '', $table = '')
    {
        $input = input('param.');
        $column = array(
            'name' => $name,
            'title' => $title,
            'type' => $type ? $type : 'text',
            'default' => $default ? $default : $input[$name],
            'param' => $param,
            'class' => $class,
            'table' => $table
        );
        ...
        $this->variable['top_search'][] = $column;
        return $this;
    }
     ...
     */
    public function setTopSearch($columns)
    {
        if (!empty($columns)) {
            foreach ($columns as $column) {
                call_user_func_array([$this, 'addSearchColumn'], $column);
            }
        }
        return $this;
    }
    ...
    public function bottom_button_select($button = [], $filter = [],$table='')
    {

        $this->variable['bottom_button_select'] = $button;
        return $this;

    }

    public function top_statistics($button = [], $filter = [])
    {

        $this->variable['top_statistics'] = $button;
        return $this;
    ...
    {
        $this->variable['row_list'] = $this->toArray($this->variable['row_list']);
            ...
            if ($this->variable['right_button']) {
                ...
                foreach ($this->variable['right_button'] as $index => $button) {
                    ...
                    if ($row['status'] == 0) {
                        if ($button['title'] == '禁用') {
                            $button['title'] = '启用';
                            $button['class'] = 'btn btn-xs mr5 btn-default  ajax-get confirm';
                    ...
                    switch ($column['type']) {
                        ...
                        case 'rightlayer': // 右侧弹层
                            if ($column['default'] != '') {
                                ...
                                $row[$column['name'] . '__' . $column['type']] = '<a data-url="' . $url . '?layer=1"
                                    data-toggle="dialog-right"
                                    title="' . $row[$column['name']] . '"
                        ...
                        case 'select': // 下拉框
                            if ($column['param']) {
                                $prepend = isset($column['param'][$row[$column['name']]]) ? $column['param'][$row[$column['name']]] : '无对应值';
                                $class =  'select-edit';
                                $source = json_encode($column['param'], JSON_FORCE_OBJECT);
                                $row[$column['name'] . '__' . $column['type']] = '<a href="javascript:void(0);" 
                                    class="' . $class . '"
                                    data-table="' . $this->createTableToken($_table_name == '' ? $this->_table_name : $_table_name, $this->_prefix) . '" 
                                    data-type="select" 
                        ...
                        case 'text.edit': // 可编辑的单行文本
                            ...
                            break;
                        case 'text.tip': // 可提示的单行文本
                            $row[$column['name']] = str_replace("\n","",$row[$column['name']]);
                            $row[$column['name'] . '__' . $column['type']] = '<span 
                            class="text-tip" 
                            id="texttip' . $row['_primary_key_value'].$column['name'] . '"
                            onmouseover="texttips(\'' . $row[$column["name"]] . '\',\'texttip' . $row['_primary_key_value'].$column['name'] . '\')" 
                            data-name="' . $_name . '">' . $row[$column['name']] . '</span>';
                            break;
                        default: // 默认
                            // 设置默认值
    ...
    public function fetch($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        // 实例化视图并渲染
        return parent::fetch($this->variable['template'], $this->variable, $replace, $config);
    }
