...
class Tree
{
    ...
    public static function reSorts($data, $pid = 0)
    {
        $ret = array();
        foreach ($data as $k => $v) {
            if ($v['pid'] == $pid) {
                if($pid != 0){
                    $v['children'] = self::reSorts($data, $v['id']);
                }
    ...
    public static function toLayer($lists = [], $pid = 0, $max_level = 0, $curr_level = 0)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value[self::$config['pid']] == $pid) {
                ...
                $trees[$value[self::$config['id']]] = $value;
            }
        }
        return $trees;
    }
     ...
     */
    public static function toList($lists = [], $pid = 0, $level = 0, $max_level = 0)
    {
        if (is_array($lists)) {
            $trees = [];
            foreach ($lists as $key => $value) {
                if ($value[self::$config['pid']] == $pid) {
                    ...
                    $trees[] = $value;
                    unset($lists[$key]);
                    $trees = array_merge($trees, self::toList($lists, $value[self::$config['id']], $level + 1, $max_level));
                }
            }
            return $trees;
        } else {
            foreach ($lists as $key => $value) {
                if ($value[self::$config['pid']] == $pid && is_object($value)) {
                    ...
                    $lists[] = $value;
                    self::toList($lists, $value[self::$config['id']], $level + 1, $max_level);
                }
            }
            return $lists;
        }
