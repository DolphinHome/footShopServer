...
class BasicWeChat
{
     ...
     * @throws \WeChat\Exceptions\LocalCacheException
     */
    protected function httpPostForJson($url, array $data, $buildToJson = true, $options = [])
    {
        try {
            return Tools::json2arr(Tools::post($url, $buildToJson ? Tools::arr2json($data) : $data, $options));
        } catch (InvalidResponseException $e) {
            if (in_array($e->getCode(), ['9410000'])) {
                //小程序直播间没有数据
                $return = [
                    'errcode' => $e->getCode(),
                    'errmsg' => '没有数据'
            ...
            throw new InvalidResponseException($e->getMessage(), $e->getCode(), $e->raw);
        }
    }
     ...
     * @throws \WeChat\Exceptions\LocalCacheException
     */
    public function callPostApi($url, array $data, $isBuildJson = true, $options = [])
    {
        $this->registerApi($url, __FUNCTION__, func_get_args());
        return $this->httpPostForJson($url, $data, $isBuildJson, $options);
    }
