...
class BasicWePay
{
    ...
    public function _getPaySign(array $data, $signType = 'MD5', $buff = '')
    {
        ksort($data);
        if (isset($data['sign'])) unset($data['sign']);
        foreach ($data as $k => $v) $buff .= "{$k}={$v}&";
...
//            $option['ssl_key'] = $this->config->get('ssl_key');
            $option['ssl_p12'] = '/www/wwwroot/powerful.jishu11.com/addons/WeChat/sdk/cert/apiclient_cert.p12';
            $option['ssl_cer'] = '/www/wwwroot/powerful.jishu11.com/addons/WeChat/sdk/cert/apiclient_cert.pem';
            $option['ssl_key'] = '/www/wwwroot/powerful.jishu11.com/addons/WeChat/sdk/cert/apiclient_key.pem';
