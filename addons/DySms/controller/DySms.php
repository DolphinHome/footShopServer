...
use addons\DySms\model\DySms as SmsModel;
...
class DySms extends Common
{
    ...
    public static function getAcsClient() {
        ...
        // AccessKeyId
        $accessKeyId = addons_config('DySms.appkey');

        // AccessKeySecret
        $accessKeySecret = addons_config('DySms.secret');
     ...
     *
     * 示例：
     * $result = addons_action('DySms/DySms/send', ['手机号码', [模板变量], '模板名称']);
     * if($result['code']){
    ...
    public static function send($rec_num = '', $sms_param = [], $sms_template = '', $sms_extend = '123456') {
        // 插件配置参数
        $config = addons_config('DySms');
        if ($config['status'] != '1') {
            throw new \Exception("短信功能已关闭");
        }
        if ($config['appkey'] == '') {
           throw new \Exception( '请填写APPKEY');
        }
        if ($config['secret'] == '') {
           throw new \Exception('请填写SECRET');
        }
        if ($rec_num == '') {
           throw new \Exception('请填写短信接收号码');
        }
        if ($sms_template == '') {
        ...
        if ($template['status'] == '0') {
            throw new \Exception( '短信模板已禁用');
        }
        if ($template['code'] == '') {
            throw new \Exception( '请设置模板ID');
        }
        if ($template['sign_name'] == '') {
            throw new \Exception( '请设置短信签名');
        }
        ...
        if (!empty($sms_param)) {
            ...
            // 替换参数          
            $keys = array_keys($sms_param);
            foreach ($keys as $v){
                $content = str_replace('${'.$v.'}',$sms_param[$v], $content);
            }
        }
...
//        $request->setOutId("yourOutId");
        ...
        if ($acsResponse->Code != 'OK') {
            // 发送失败
            $msg = isset($acsResponse->Message) ? $acsResponse->Message : $acsResponse->Message;          
            throw new \Exception( $acsResponse->Code . ":" . $msg);
        } 
        return $content;   
    }
    ...
    public function sendBatch($rec_nums = [], $sms_params = [], $sms_template = '')
    {
        ...
        if (count($rec_nums) > 100) {
           throw new \Exception('批量上限为100个手机号码');
        }
        if (empty($sms_params)) {
            throw new \Exception( '请填写短信模板变量');
        }
        if ($sms_template == '') {
           throw new \Exception( '没有设置短信模板');
        }
