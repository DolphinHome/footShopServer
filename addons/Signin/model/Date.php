    ...
    public static function unixtime($type = 'day', $offset = 0, $position = 'begin', $year = null, $month = null, $day = null, $hour = null, $minute = null)
    {
        ...
        switch ($type) {
            ...
            case 'month':
                ...
                $month_v = $month + $offset;
                $time = $position ? mktime(0, 0, 0, $month + $offset, 1, $year) : mktime(23, 59, 59, $month + $offset, date("t", strtotime("{$year}-{$month_v}")), $year);
                break;
            case 'quarter':
                /*$time = $position ?
                ...
                $time = $position ?
                    mktime(0, 0, 0, 1 + ((ceil(date('n', mktime(0, 0, 0, $month, $day, $year)) / 3) + $offset) - 1) * 3, 1, $year) :
                    mktime(23, 59, 59, (ceil(date('n', mktime(0, 0, 0, $month, $day, $year)) / 3) + $offset) * 3, date("t", strtotime("{$year}-{$month}")), $year);
                break;
            case 'year':
