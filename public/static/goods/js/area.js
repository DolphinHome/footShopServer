    ...
    RegionalChoice.prototype = {
        ...
        setChecked: function (checkedIds) {
            ...
            $.each(checkedIds, function (i, id) {
                ...
                , $siblings = $p.siblings();
                $siblings.length > 0 ? $p.css('display','block') : $p.closest('.place-tooltips').css('display','block');
            });
        },
        ...
        setAlready: function (alreadyIds) {
            ...
            $.each(alreadyIds, function (i, id) {
                var $p = $place.find('#' + id).parent().parent()
                    , $siblings = $p.siblings();
                $siblings.length > 0 ? $p.css('display','none') : $p.closest('.place-tooltips').css('display','none');
            });
        },
