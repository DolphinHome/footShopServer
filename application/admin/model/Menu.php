...
class Menu extends ThinkModel
{
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        if (!$menus) {
            ...
            $quick_menus = Quick::alias('q')->leftJoin('admin_menu am', 'q.menu_id = am.id')->column('am.id,q.pid,am.module,am.title,am.url_value,am.url_target,am.icon,am.params');
            $menus = array_merge($menus, $quick_menus);
    ...
    public static function getLocation($id = '', $del_last_url = false, $check = true)
    {
        ...
        if ($id != '') {
            ...
            $cache_name = 'location_menu_' . $model . '_' . $controller . '_' . $action;
        }
...
// echo $curr_id.'<br>';
            ...
            $controller = request()->controller();
            if ($check && empty($location) && $controller != 'Login') {
     ...
     * 根据当前url查找menu表的id
     */
    public function getMenusByUrl($url)
    {
        $id = self::where('url_value', $url)->value('id');
        return $id??0;
    }
}
