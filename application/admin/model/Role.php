...
class Role extends Model
{
    ...
    public static function checkAuth($id = 0, $url = false)
    {
        ...
        if (session('admin_auth.uid') == '1' || $role == '1') {
            return true;
        }
    ...
    {
        $menu_auth = cache('role_menu_auth_'.session('admin_auth.role'));
        if (!$menu_auth) {
            $menu_auth = self::where('id', session('admin_auth.role'))->value('menu_auth');
        ...
        if (config('develop_mode') == 0) {
            cache('role_menu_auth_'.session('admin_auth.role'), $menu_auth);
        }
        return $menu_auth;
    }
