...
class Module extends Base
{
    ...
    public function index($group = 'local')
    {
        ...
        switch ($group) {
            case 'local':
                ...
                $this->assign('page_title', lang('模块管理'));
    ...
    public function install($name = '', $confirm = 0)
    {
        ...
        if ($name == '') $this->error(lang('模块不存在'));
        if ($name == 'admin') $this->error(lang('禁止操作系统核心模块'));
        ...
        if ($confirm == 0) {
            ...
            $this->assign('page_title', lang('安装模块').'：' . $name);
            return $this->fetch();
        }
            ...
            if (!empty($sql_statement)) {
                foreach ($sql_statement as $value) {
                    try {
                        ...
                        $this->error(lang('导入SQL失败，请检查install.sql的语句是否正确'));
                    }
                }
            }
        }
        ...
        if ($ModuleModel->allowField($allowField)->save()) {
            ...
            $this->success(lang('模块安装成功'), 'index');
        } else {
            MenuModel::where('module', $name)->delete();
            $this->error(lang('模块安装失败'));
        }
    }
    ...
    {
//        $this->error('测试版本中，禁止卸载');
        if ($name == '') $this->error(lang('模块不存在'));
        if ($name == 'admin') $this->error(lang('禁止操作系统模块'));
        ...
        if (ModuleModel::where('name', $name)->delete()) {
            ...
            $this->success(lang('模块卸载成功'), 'index');
        } else {
            $this->error(lang('模块卸载失败'));
        }
    }
    ...
    {
        $name == '' && $this->error(lang('缺少模块名'));

        $Module = ModuleModel::get(['name' => $name]);
        !$Module && $this->error(lang('模块不存在，或未安装'));
        ...
        if (false !== $Module->save($module_info)) {
            $this->success(lang('模块配置更新成功'));
        } else {
            $this->error(lang('模块配置更新失败，请重试'));
        }
    }
    ...
    {
        if ($name == '') $this->error(lang('缺少模块名'));

        $export_data = $this->request->get('export_data', '');
        if ($export_data == '') {
            $this->assign('page_title', lang('导出模块').'：'. $name);
            return $this->fetch();
        }
...
INFO;
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $module['system_module'] == 1 && $this->error(lang('禁止操作系统内置模块'));
    ...
    private function checkDependence($type = '', $data = [])
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $need[$key] = [
                $type => $value[0],
                'identifier' => $value[1],
                'version' => $curr_version ? $curr_version : lang('未安装'),
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!File::mk_dir(APP_PATH . $data['name'])) {
                $this->error(lang('创建模块') . $data['name'] . '目录失败,请检查权限或是否已存在');
            }
            ...
            $c_file_content = File::read_file(ROOT_PATH . 'data/page/api.tpl');
            $c_file_content = str_replace(['{module}', '{model}', '{title}', '{fields}', '{fields_add}'], [$data['name'], 'Index', $data['title'], '', ''], $c_file_content);
            if (!File::write_file($c_file_dir, $c_file_content)){
        ...
        $fields = [
            ['type' => 'text', 'name' => 'name', 'title' => lang('模块标识'), 'tips' => '请使用英文字母，根据标识会生成唯一模块标识', 'attr' => 'data-rule="required;name;" data-rule-name="[/^[a-zA-Z]*$/, \'请输入正确的标识，只能使用英文，请勿使用符号和数字\']" data-msg-required="标识不能为空"'],
            ['type' => 'text', 'name' => 'title', 'title' => lang('模块名称'), 'tips' => '', 'attr' => '', 'value' => '', 'attr' => 'data-rule="required;"'],
            ['type' => 'icon', 'name' => 'icon', 'title' => lang('图标'), 'tips' => lang('点击后请选择图标')],
            ['type' => 'textarea', 'name' => 'description', 'title' => lang('简介')],
            ['type' => 'text', 'name' => 'author', 'title' => lang('作者')],
            ['type' => 'text', 'name' => 'version', 'title' => lang('版本号'), 'tips' => lang('请填写三段式版本号'), 'attr' => '', 'value' => '1.0.0', 'attr' => 'data-rule="required;"'],

        ];
        $this->assign('page_title', lang('创建模块'));
