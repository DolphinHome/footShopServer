...
class Menu extends Base
{
    ...
    public function index($group = 'admin')
    {
        ...
        $this->assign('page_title', lang('菜单管理'));
        return $this->fetch();
    }
    ...
    public function add($module = 'admin', $pid = '')
    {
		...
		$fields =[
			...
			['type' => 'select', 'name' => 'module', 'title' => lang('所属模块'), 'extra' =>  ModuleModel::getModule(), 'value' => $module, 'ajax_url' => url('menu/getModuleMenus')],
			['type' => 'select', 'name' => 'pid', 'title' => lang('所属菜单'), 'extra' => MenuModel::getMenuTree(0, '', $module), 'value' => $pid ],
			[
				'type' => 'text', 
				'name' => 'title', 
				'title' => lang('菜单标题'), 
            ...
            ['type' => 'text', 'name' => 'url_value', 'title' => lang('菜单链接') ,'tips' => '可留空，如果是模块链接，请填写 "模块/控制器/操作"'],
			['type' => 'icon', 'name' => 'icon', 'title' => lang('菜单图标') ,'tips' => '例如： fa fa-fw fa-user'],
			['type' => 'radio', 'name' => 'auto_create', 'title' => lang('自动添加子菜单'), 'extra' => [lang('否'), lang('是')], 'value' => 0, 'tips' => '子菜单包含新增、编辑、设置状态、删除'],
			['type' => 'select', 'name' => 'role', 'title' => lang('角色'), 'extra' => RoleModel::where('id', 'neq', 1)->column('id,name'), 'attr' => 'multiple'], 
            ['type' => 'radio', 'name' => 'online_hide', 'title' => lang('是否隐藏'), 'extra' => [lang('否'), lang('是')], 'value' => 0],
            ['type' => 'text', 'name' => 'sort', 'title' => lang('排序'), 'value' => '99'],
            ['type' => 'radio', 'name' => 'status', 'title' => lang('状态'), 'extra' => [lang('禁用'), lang('启用')], 'value' => 1]
		];
		$this->assign('page_title',lang('新增菜单'));
    ...
    public function edit($id = 0)
    {
		...
		$fields =[
			['type' => 'hidden', 'name' => 'id' ],
			['type' => 'linkage', 'name' => 'module', 'title' => lang('所属模块'), 'extra' =>  ModuleModel::getModule(), 'ajax_url' => url('menu/getModuleMenus'), 'next_items' => 'pid'],
			['type' => 'select', 'name' => 'pid', 'title' => lang('所属菜单'), 'extra' => MenuModel::getMenuTree(0, '', $info['module'])],    
			[
				'type' => 'text', 
				'name' => 'title', 
				'title' => lang('菜单标题'), 
			...
			['type' => 'icon', 'name' => 'icon', 'title' => lang('菜单图标') ,'tips' => '例如： fa fa-fw fa-user'],
			['type' => 'select', 'name' => 'role', 'title' => lang('角色'), 'extra' => RoleModel::where('id', 'neq', 1)->column('id,name'), 'attr' => 'multiple'], 
            ['type' => 'radio', 'name' => 'online_hide', 'title' => lang('是否隐藏'), 'extra' => [lang('否'), lang('是')]],
            ['type' => 'text', 'name' => 'sort', 'title' => lang('排序')],
            ['type' => 'radio', 'name' => 'status', 'title' => lang('状态'), 'extra' => [lang('禁用'), lang('启用')], 'value' => 1]
		];
		$this->assign('page_title',lang('编辑菜单'));
    ...
    public function getNestMenu($lists = [], $max_level = 0, $pid = 0, $curr_level = 1)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value['pid'] == $pid) {
                ...
                $result .= '<a href="'.url('add', ['module' => $value['module'], 'pid' => $value['id'],'layer'=>1]).'" class="btn btn-default btn-xs layeradd">新增</a> <a href="'.url('edit', ['id' => $value['id'],'layer'=>1]).'" data-original-title="编辑" class="btn btn-default btn-xs layeredit">编辑</a> ';
                if ($value['status'] == 0) {
                    // 启用
    ...
    public function getSidebarMenu($module_id = '', $module = '', $controller = '')
    {
        ...
        foreach ($menus as $key => $menu) {
            ...
            if (!empty($menu['child'])) {
				...
				$output1 = current($menu['child']);
                $output = $output1['url_value'];
                break;
            }
        }
        return $output;
    ...
    public function setStatus($type = '')
    {
        ...
        //父级菜单禁用，子菜单也禁用
        $child_ids =   MenuModel::getChildsId($ids[0]);
        $ids = array_merge($ids,$child_ids);
       
        switch ($type) {
            case 'disable': // 禁用
                $result = MenuModel::where('id','IN',$ids)->setField('status', 0);
                break;
            case 'enable': // 启用
                $result = MenuModel::where('id','IN',$ids)->setField('status', 1);
                break;
			case 'delete': // 删除
				    $result = MenuModel::where('id','IN',$ids)->delete();
				    break;
            default:
