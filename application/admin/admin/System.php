...
class System extends Base
{
    ...
    public function index($group = 'base')
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->success(lang('更新成功'), url('index', ['group' => $group]));
        } else {
            // 配置分组信息
            ...
            foreach ($modules as $name => $module) {
                $list_group[$name] = lang($module['title'])??$module['title'];
            }

            $tab_list   = [];
            foreach ($list_group as $key => $value) {
                $tab_list[$key]['title'] = lang($value)??$value;
            ...
            if (isset(config('config_group')[$group])) {
                ...
                foreach ($data_list as &$value) {
                    ...
                    //配置里边的lang
                    $value['title'] = lang($value['title'])??$value['title'];
                    $value['tips'] = lang($value['tips'])??$value['tips'];
                }

				// 默认模块列表
                if (isset($data_list['home_default_module'])) {
                    $list_module['index'] = lang('默认');
