 ...
 * @LastEditors: wangph
 * @LastEditTime: 2021-04-28 14:39:09
 */
...
class ActivityDetails extends Base
{
    ...
    public function index()
    {
        ...
        if (input('param.activity_id')) {
            ...
            $top_add[] = ['title'=>lang('批量删除'),'href'=>'delete','icon'=>'fa fa-times pr5','class'=>'btn btn-sm mr5 btn-default  ajax-post confirm','extra'=>'target-form="ids"'];
...
//            ['member_activity_price', lang('会员价格'),'text.edit'],//['activity_price', lang('活动价格'),'text.edit']
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                foreach ($data['sku_id'] as $k => $v) {
                    ...
                    if ($v) {
                        ...
                        if ($data['price'][$k] > $goods['shop_price']) {
                            exception(lang('请设置第') . $j . lang('个规格商品的活动价格小于出售价格'));
                        }
                    ...
                    //$res = Db::name('goods_sku')->where([['sku_id', '=', $v]])->setDec('stock', $data['stock'][$k]);
                        ...
                        if ($data['price'][$k] > $goods['shop_price']) {
                            exception(lang('规格商品的活动价格小于出售价格'));
                        }
                        $savedata['stock'] = $data['stock'][$k];
                        $savedata['limit'] = $data['limit'][$k] ?? 1;
                    ...
                    if ($data['activity_type'] == 3) {
                        if ($data['price'][$k] >= $data['member_price'][$k]) {
                            exception(lang('定金不能超过商品价格'));
                        }
                    ...
                    if ($data['activity_type'] == 10) {
                        $savedata['bargain_max'] = $data['bargain_max'][$k];
                        $savedata['bargain_min'] = $data['bargain_min'][$k];
                        $savedata['least_count'] = $data['least_count'][$k];
                    }
                    /*$savedata['member_activity_price'] = $data['member_price'][$k] ? $data['member_price'][$k] : $data['price'][$k];*/
                    $savedata['member_activity_price'] = $data['member_price'][$k] ? $savedata['activity_price'] : $savedata['activity_price'];
            ...
            action_log('goods_activity_details_add', 'goods_activity_details', $goods_activity_details_id, UID, $details);
     ...
     * @author jxy [ 41578218@qq.com ]
     */
    public function getGoodsByCid($activity_id)
    {
        if (!$activity_id) {
            echo json_encode(['code' => 0, 'msg' => lang('缺少活动ID')]);
            exit;
        }
        $Activity = ActivityModel::where([['id', '=', $activity_id]])->find();
        if (!$Activity) {
            echo json_encode(['code' => 0, 'msg' => lang('活动不存在')]);
            exit;
        }
        if ($Activity['cid'] > 0) {
            $cid = \app\goods\model\Category::getChildsId($Activity['cid']);
            if (count($cid)) {
                $map[] = ['cid', 'in', $cid];
            } else {
                $map[] = ['cid', '=', $Activity['cid']];
            }
        }
        /**如果是拼团活动添加活动商品必须保证普通商品有库存**/
        if ($Activity['type'] == 2) {
            $map[] = ['stock', '>', 0];
        }

        //参加过活动的商品不再显示
        $goods_id = ActivityDetailsModel::where([
            ['activity_id', '=', $activity_id],
            ['status', '=', 1],
        ])->column("goods_id");
        if (count($goods_id) > 0) {
            $map[] = ['id', 'not in', $goods_id];
        }
        ...
        echo json_encode(['code' => 1, 'msg' => lang('请求成功'), 'list' => $list, 'activity_type' => $Activity['type']]);
        exit;
    }
    ...
    {
        if (!$sku_id) {
            echo json_encode(['code' => 0, 'msg' => lang('缺少规格ID')]);
            exit;
        }
        $sku = Db::name('goods_sku')->find($sku_id);
        echo json_encode(['code' => 1, 'msg' => lang('请求成功'), 'data' => $sku]);
    }
    ...
    {
        if (!$goods_id) {
            echo json_encode(['code' => 0, 'msg' => lang('缺少商品ID')]);
            exit;
        }
        ...
        if ($goods['is_delete'] == 0 && $goods['status'] == 1) {
            if ($goods['is_spec'] == 1) {
                $list = Db::name('goods_sku')->where([['goods_id', '=', $goods_id], ['status', '=', 1]])->select();
                echo json_encode(['code' => 1, 'msg' => lang('请求成功'), 'list' => $list, 'is_goods' => 0]);
                exit;
            } else {
                $list[] = [
                ...
                echo json_encode(['code' => 1, 'msg' => lang('请求成功'), 'list' => $list, 'is_goods' => 1]);
                exit;
            }
        } else {
            echo json_encode(['code' => 0, 'msg' => lang('商品已删除或禁用'), 'list' => []]);
            exit;
        }
    }
