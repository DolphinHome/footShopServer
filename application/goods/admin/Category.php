...
use addons\ExpressBird\controller\Api;
use app\admin\admin\Base;
use app\goods\model\Category as CategoryModel;
...
class Category extends Base
{
    ...
    {
        $data = CategoryModel::where('status = 1')->field('id,name,pid,is_hot,level')->select()->toArray();
        $list = $this->CategoryList($data);
...
//        ])
    ...
    public function saveCate()
    {
        ...
        if (mb_strlen($data['name'], 'utf8') > 10) {
            return ApiReturn::r(0, [], lang('分类名称不能超过十个字'));
        }
    ...
    public function getCate()
    {
        ...
        $data_list = $this->generateTree($data_list, 'goods_cate');
        /*foreach ($data_list as &$v) {
            ...
            foreach ($v['children'] as &$m) {
                ...
                foreach ($m['children'] as &$n) {
                    ...
                    foreach ($n['children'] as &$v) {
                        $v['thumb_img'] = get_file_url($v['thumb']);
                        foreach ($v['children'] as &$o) {
                            $o['thumb_img'] = get_file_url($o['thumb']);
                        }
                    }
                }
            }
    ...
    public function del()
    {
        ...
        if ($find) {
            return ApiReturn::r(0, [], '当前分类下有下级分类，不可以删除');
        }
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($res = CategoryModel::create($data)) {
                //记录行为
                unset($data['__token__']);
                $details = json_encode($data, JSON_UNESCAPED_UNICODE);
                action_log('goods_category_add', 'goods_category', $res->id, UID, $details);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (CategoryModel::update($data)) {
                ...
                action_log('goods_category_edit', 'goods_category', $id, UID, $details);
