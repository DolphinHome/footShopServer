...
class Role extends Model
{
    ...
    public static function getTree($id = null, $default = '', $filter = null)
    {
        ...
        // 获取菜单
        $roles = self::where('status', 1)->where($where)->column('id,pid,name');
        $pid   = self::where('status', 1)->where($where)->order('pid')->value('pid');
    ...
    {
        // 获取当前用户的权限
        $menu_auth = session('user_role_menu_auth');
    ...
    {
        $menu_auth = cache('user_role_menu_auth_'.session('mall_user_auth.user_auth'));
        if (!$menu_auth) {
            $menu_auth = self::where('id', session('mall_user_auth.user_auth'))->value('menu_auth');
            $menu_auth = json_decode($menu_auth, true);
            $menu_auth = MenuModel::where('aid', 'in', $menu_auth)->column('aid,url_value');
        }
        
        // 非开发模式，缓存数据
        if (config('develop_mode') == 0) {
            cache('user_role_menu_auth_'.session('mall_user_auth.user_auth'), $menu_auth);
        }
        return $menu_auth;
    }
